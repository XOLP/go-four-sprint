...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpedMsec := meanSpeed(action, duration) * kmhInMsec
	heightInM := height / cmInM

	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpedMsec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH) // средную скорость перевести в м/с
